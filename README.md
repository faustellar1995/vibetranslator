...
- **全局快捷键**：`Ctrl+F2` / `Alt+F2` 翻译当前窗口选中的文字；无选中时自动翻译剪贴板内容
- **自动模式**：勾选后每隔设定秒数（默认 3s，可调 1-60s）轮询当前选中内容，与上次翻译原文不同时自动翻译
...
1. 设置 API Key（二选一）：
   ...
   - 打开主界面 → 输入 Key → 点「保存」
2. 选中任意窗口中的文字，按 `Ctrl+F2` 或 `Alt+F2`
3. 译文气泡跟随鼠标显示：**点击鼠标**关闭，按 **Ctrl+C** 复制译文
4. 需要持续跟译时，打开主界面勾选「自动模式」并设定轮询间隔（默认 3 秒）
